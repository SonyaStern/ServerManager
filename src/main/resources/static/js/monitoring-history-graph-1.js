function loadHistoryGraph(elementId) {
    // element = elementId
...
// Adds the svg canvas
    ...
    let minTime;
    let maxTime;
    d3.json("/get-cpu-history")
        .row(function(d) { return { time: parseDate(d.key), load: d.value}; })
        .get(function(error, rows) {
            max = d3.max(rows, function (d) {
                return d.load;
            });
            minTime = d3.min(rows, function (d) {
                return d.time;
            });
            maxTime = d3.max(rows, function (d) {
                return d.time;
            });
        });
